...
#include <mutex>
#include <cassert>
#include <cuda_runtime.h> // Linked directly to utilize real host GPU resources
...
struct [[nodiscard]] AllocationResult {
    void* physical_gpu_ptr{nullptr};
    uint32_t block_index{0};
    AllocatorStatus status{AllocatorStatus::Success};
};

// ============================================================================
// 2. HIGH-FREQUENCY O(1) BITMAPPED SLAB ALLOCATOR
// ============================================================================
class VramSlabAllocator {
public:
    // RAII Pre-Allocation: Reserve the full backing pool footprint once at startup
    VramSlabAllocator() noexcept 
        : base_vram_pool_ptr_(nullptr), 
          allocation_bitmap_(0xFFFFFFFFFFFFFFFFULL) // All 64 bits set to 1 (1 = Available, 0 = Occupied)
    {
        ...
        cudaError_t err = cudaMalloc(&base_vram_pool_ptr_, total_arena_bytes);
        if (err != cudaSuccess) {
            std::cerr << "[Hardware Fault] Critical initialization failure on cudaMalloc: " 
    ...
    ~VramSlabAllocator() noexcept {
        if (base_vram_pool_ptr_) {
            cudaFree(base_vram_pool_ptr_);
        }
    }

    // Eliminate copy operations to maintain unified single-point resource tracking boundaries
    VramSlabAllocator(const VramSlabAllocator&) = delete;
    VramSlabAllocator& operator=(const VramSlabAllocator&) = delete;
    ...
    [[nodiscard]] AllocationResult AllocateTensorBlock() noexcept {
        if (!base_vram_pool_ptr_) [[unlikely]] {
        ...
        // std::countr_zero returns the number of consecutive zero bits starting from the right.
        // Since available blocks are 1, we count trailing zeros of the inversion (~bitmap) to find the first 1.
        ...
        uint8_t* byte_ptr = reinterpret_cast<uint8_t*>(base_vram_pool_ptr_);
    ...
    [[nodiscard]] AllocatorStatus FreeTensorBlock(void* block_ptr) noexcept {
        if (!block_ptr || !base_vram_pool_ptr_) [[unlikely]] {
        ...
        uint8_t* base_ptr = reinterpret_cast<uint8_t*>(base_vram_pool_ptr_);
...
// ============================================================================
int main() {
    std::cout << "=== RUNNING HIGH-PERFORMANCE VRAM SLAB ALLOCATOR ===\n\n";

    VramSlabAllocator allocator;
    ...
    // Step 1: Simulate rapid allocation requests for deep learning tensors
    std::cout << "[Allocation Phase] Extracting two 2MB tensor blocks from VRAM...\n";
    ...
    if (tensor_a.status != AllocatorStatus::Success || tensor_b.status != AllocatorStatus::Success) {
        std::cerr << "Hardware allocation crash. Check cloud GPU properties.\n";
        return -1;
    }

    std::cout << "  -> Tensor A Assigned Block Index = " << std::dec << tensor_a.block_index 
              << " | Hardware Address = 0x" << std::hex << reinterpret_cast<uintptr_t>(tensor_a.physical_gpu_ptr) << "\n";
    std::cout << "  -> Tensor B Assigned Block Index = " << std::dec << tensor_b.block_index 
              << " | Hardware Address = 0x" << std::hex << reinterpret_cast<uintptr_t>(tensor_b.physical_gpu_ptr) << "\n";
    ...
    AllocatorStatus free_status = allocator.FreeTensorBlock(tensor_a.physical_gpu_ptr);
    ...
    std::cout << "=== ALL SLAB ALLOCATOR VALIDATIONS GRACEFULLY PASSED ===\n";
